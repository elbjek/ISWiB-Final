This is a template website for ISWiB -International Student Week in Belgrade done as a final project at SAE institute.

#  Final project
Requirements:

- Create a responsive layout using the Principles of Design of this Module
- Use the Gridsystem to create the layout of the website:
    Website is responsive with help of bootstrap grid.



- Present the Mockup of the website

You can see the mockup of the website here https://invis.io/VAGR7Y4DFRQ


- Javascript Validation Form with styled feedback to the user (validation for name length, email format, telephone number, age etc);

Validated contact page.

- Javascript Image Gallery with image handling such as image rotation, overview, slider options (autoplay, pause, timing options)
Gallery page.

- At least two of the photos in Image Gallery must be retouched demonstrating the differences (before & after)
Hero section and Our Team section on the landing page.  Our Team section (on landing page) has collapsable information.
Unedited 

 - Use of AJAX technique for loading data from XML/JSON into the website

Weather information on top is an AJAX request.

- At least one vector image with css styling
Logo, and 3 icons in Activities section, on landing page are animated.

///////////////


- Use Javascript properties to identify the browser & the compatibility with the Website


JS validated form, Scroll to top and shrinking navigation 




<!-- A PDF and README.md or README.txt that detail the layouts (responsive) and the JS techniques followed on the website -->